    ...
    <main className="mx-auto max-w-6xl px-5 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {["Loops continuously", "Pauses off-screen", "Reduced-motion fallback", "L1–L4 labels", "No private keys, ever"].map(
          (t) => (
            <span key={t} className="pill !text-xs !uppercase !tracking-wider">
              {t}
            </span>
          ),
        )}
      ...
      */}
      <div className="card mt-8 p-5">
        <div className="font-mono text-xs uppercase tracking-[0.2em] text-amber">What these are, and are not</div>
      ...
      {groups.map(({ group, entries }) => (
        <section key={group} className="mt-12">
          <div className="font-mono text-xs uppercase tracking-[0.22em] text-muted">{group}</div>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {entries.map((e) => (
              ...
              >
                ...
                <div className="mt-3 text-lg font-medium leading-tight tracking-tight">{e.title}</div>
                <p className="mt-2 text-sm leading-relaxed text-muted text-pretty">{e.summary}</p>
              </Link>
            ))}
          </div>
        </section>
