/**
 * POST /api/dns/verify — real DNS TXT lookup for `_agenid.<domain>`.
 *
 * Same verification logic as /api/verify-dns but at the new /api/dns/ path
 * for consistency with the dns/detect and dns/auto-add routes.
 *
 * Proves DOMAIN CONTROL and nothing more. Domain control is evidence an
 * authority would weigh when issuing a VerificationAssertion; it is not
 * itself a verification level.
 */
